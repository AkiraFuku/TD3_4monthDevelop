...
void BaseGameScene::Initialize() {
  ...
  //    Audio::GetInstance()->PlayAudio(BGMhandle_, true);
    ...
    // BaseGameScene.cpp の LoadStageData() のループ内
...
void BaseGameScene::Update()
{
    ...
    ImGui::Text("isDrawPuase_ : %s", isDrawPause_ ? "True" : "False");
    ...
    if (isResetWaiting_ && fade_->IsFinished())
    {
        ...
        // ※補足：位置以外の変数（体力、発射回数、スコアなど）をリセットする必要があれば②②に書きます
        // threadLimit_ = 0; 
        // player_->ResetHP(); // 例
...
void BaseGameScene::Pause()
{
    ...
    } else
    {
        ...
        if (Input::GetInstance()->TriggerKeyDown(DIK_L))
        {
            if (isDrawPause_)
            {
                isDrawPause_ = false;
