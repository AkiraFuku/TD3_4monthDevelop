...
#include "Logger.h"
// 静的メンバ変数の初期化
std::unique_ptr<Line3dCommon> Line3dCommon::instance = nullptr;

Line3dCommon* Line3dCommon::GetInstance() {
    if (Line3dCommon::instance == nullptr) {
        Line3dCommon::instance.reset(new Line3dCommon());
    }
    return Line3dCommon::instance.get();
}

void Line3dCommon::Finalize() {
    Line3dCommon::instance.reset();
}
void Line3dCommon::Initialize()
{

    PsoConfig config{};
    config.vsPath = L"resources/shaders/Line/Line.vs.hlsl";
    config.psPath = L"resources/shaders/Line/Line.ps.hlsl";
...
void Line3dCommon::Line3dCommonDraw()
{
    // RootSignatureの設定
